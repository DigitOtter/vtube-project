...
func _init_gui(gui_node: Node):
	var control_elements: Array[GuiElements.ElementData] = []
	
	# Lighting toggle
	var toggle_lighting = GuiElements.ElementData.new()
	toggle_lighting.Name = "Toggle Lighting"
	toggle_lighting.Data = GuiElements.CheckBoxData.new()
	...
	# Lighting toggle
	var flip_lighting = GuiElements.ElementData.new()
	flip_lighting.Name = "Flip Texture H"
	flip_lighting.Data = GuiElements.CheckBoxData.new()
	...
	# Blur level
	var blur_level_limits: GuiElements.SliderData = GuiElements.SliderData.new()
	blur_level_limits.MinValue = 0.0
	blur_level_limits.MaxValue = 20.0
	blur_level_limits.Step     = 0.1
	blur_level_limits.Default  = 5.3
	
	var blur_level: GuiElements.ElementData = GuiElements.ElementData.new()
	...
	# Min Lighting color
	var min_color_limits: GuiElements.SliderData = GuiElements.SliderData.new()
	min_color_limits.MinValue = 0.0
	min_color_limits.MaxValue = 1.0
	min_color_limits.Step     = 0.05
	min_color_limits.Default  = 0.1
	
	var min_color: GuiElements.ElementData = GuiElements.ElementData.new()
	...
	gui_node.add_element_tab("External Lighting", control_elements)
...
func _ready():
	self._init_gui(get_node(Gui.GUI_NODE_PATH).get_gui_elements())
	self._init_input()
